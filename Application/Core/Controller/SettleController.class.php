...
class SettleController extends CommonController {
    ...
    public function recruitAutomaticSettlement(){
        ...
        foreach($recruit_list as &$val){
            if($val['add_time'] < $dividing_time){
                ...
                //清空悬赏剩余赏金
                $recruit_result = $recruitModel->saveRecruitData(array('id' => $val['id']), array('last_token' => 0, 'status' => 0));
    ...
    /**
     * @desc 任务系统开启，月末自动结算任务奖金
     * @extra 未执行手动修改参数结算
     */
    public function taskMonthlySettlement(){
        $settlement_amount = C('TASK_REWARD');//结算金额，可手动修改
        $settle_rate = C('TASK_RATE');//手动结算修改比例格式：'50|20|30' 简历上传比例|招聘占比|推荐占比
        $is_on = C('IS_TASK');//任务系统是否开启
        if($is_on){
            $year = date('Y');//获取年份
            $month = date('m');//获取月份
            $days = date('t', strtotime($year.'-'.$month));//总天数
            $start = mktime(0, 0, 0, $month, 1, $year);
            $end = mktime(23, 59, 59, $month, $days, $year);
            //分组结算
            $settle_rate = explode('|', $settle_rate);
            $resume_amount = $settlement_amount * $settle_rate[0];
            $recruit_amount = $settlement_amount * $settle_rate[1];
            $recommend_amount = $settlement_amount * $settle_rate[2];
            //简历上传
            $task_model = M('TaskLog');
            $task_id = 3;
            $task_where = array('finish_time' => array('between', array($start, $end)), 'task_id' => $task_id);
            $task_field = 'user_id,count(user_id) as user_count';
            $total_task_data = $task_model->where($task_where)->count();
            $task_log_data = $task_model->where($task_where)->field($task_field)->group('user_id')->select();
            $user_model = M('User');
            $account_log = M('AccountLog');
            foreach($task_log_data as &$val){
                $check_account = $account_log->where(array('user_id' => $val['user_id'], 'change_type' => 11, 'change_time' => array('between', array($start, $end))))->find();
                if($check_account) continue;
                $user_can_resume_amount = round(($val['user_count'] / $total_task_data), 4) * $resume_amount;
                $user_model->where(array('user_id' => $val['user_id']))->setInc('user_money', $user_can_resume_amount);
                account_log($val['user_id'], $user_can_resume_amount, 11, '简历上传奖金');
            }
            unset($val);
            //推荐/招聘结算
            $recommend_model = M('RecruitRecommend');
            $recruit_recommend_where = array('finish_time' => array('between', array($start, $end)));
            $total_recruit_field = 'count(user_id) as total_count, operate_type';
            $total_recruit = $recommend_model->where($recruit_recommend_where)->group('operate_type')->field($total_recruit_field)->select();
            $total_recruit_count = 0;
            $total_recommend_count = 0;
            foreach($total_recruit as &$val){
                if(1 == $val['operate_type']) $total_recruit_count += $val['total_count'];
                if(2 == $val['operate_type']) $total_recommend_count += $val['total_count'];
            }
            unset($val);
            $recruit_field = 'user_id,operate_type,count(user_id) as user_count';
            $recruit_log_data = $recommend_model->where($recruit_recommend_where)->group('user_id, operate_type')->field($recruit_field)->select();
            foreach($recruit_log_data as &$val){
                if($total_recruit_count > 0){
                    if(1 == $val['operate_type']){
                        $check_account = $account_log->where(array('user_id' => $val['user_id'], 'change_type' => 13, 'change_time' => array('between', array($start, $end))))->find();
                        if($check_account) continue;
                        $user_can_recruit_amount = round($val['user_count'] / $total_recruit_count, 4) * $recruit_amount;
                        $user_model->where(array('user_id' => $val['user_id']))->setInc('user_money', $user_can_recruit_amount);
                        account_log($val['user_id'], $user_can_recruit_amount, 13, '招聘奖金');
                    }
                }
                if($total_recommend_count > 0){
                    if(2 == $val['operate_type']){
                        $check_account = $account_log->where(array('user_id' => $val['user_id'], 'change_type' => 12, 'change_time' => array('between', array($start, $end))))->find();
                        if($check_account) continue;
                        $user_can_recommend_amount = round($val['user_count'] / $total_recommend_count, 4) * $recommend_amount;
                        $user_model->where(array('user_id' => $val['user_id']))->setInc('user_money', $user_can_recommend_amount);
                        account_log($val['user_id'], $user_can_recommend_amount, 12, '推荐奖金');
                    }
                }
            }
            unset($val);
