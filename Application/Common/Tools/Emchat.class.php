...
//------------------------------------------------------用户体系    
    ...
    */
     ...
     */
    public function create_emchat_user(){
        $userName = $this->_createWebIMUserName();
        $password =$this->_createWebIMUserPwd();

        $result = $this->createUser($userName, $password); //创建环信用户
        if($result['error'] != ''){

            return array('status'=>-1,'msg'=>$result['error']);
        }
     ...
     * @param unknown $userId
     */
    private function _createWebIMUserName(){
     ...
     * @param unknown $userId
     */
    private function _createWebIMUserPwd(){
        return 'pwd'.NOW_TIME . '_' . rand(10000, 99999);
    }
}
?>