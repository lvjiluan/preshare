...
class RecruitController extends HrCommonController {
    ...
    public function editRecruit(){
        ...
        if(IS_POST){
            ...
            if($data['province']) $data['job_area'] = $data['province'].',';
            if($data['city']) $data['job_area'] .= $data['city'].',';
            if(!$data['county']) $this->ajaxReturn(V(0, '请填写详细悬赏区域'));
            if($data['county']) $data['job_area'] .= $data['county'];
            ...
            if(!$is_fee){
                $free_number_validate = $model->freeRecruitValidate($hr_id);
            ...
            if($is_fee){
                if($data['commission'] <= $getResumeMoney) $this->ajaxReturn(V(0, '悬赏金额需要大于'.$getResumeMoney.'元！'));
                $regex = '/^\d+(\.\d{1,2})?$/';
            ...
            if($recruit_id > 0){
                ...
                //修改金额
                $res['status'] = 1;
                if($is_fee) $res = $user_model->recruitUserMoney($data['commission'] * $data['recruit_num'], $hr_id);
                if ($res['status'] ==0) {
                    M()->rollback();
                    $this->ajaxReturn($res);
                }
                add_key_operation(5, $newId, HR_ID);
                $task_id = 4;
        ...
        $free_num_already = D('Admin/Recruit')->freeRecruitValidate($hr_id, true);
        $free_number = $free_num_already >= $free_num_total ? 0 : ($free_num_total - $free_num_already);
